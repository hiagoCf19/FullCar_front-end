    ...
    <>
      <div className="fixed sm:hidden bottom-0 w-full   px-4 py-2 flex bg-white dark:bg-background z-50">
        ...
        <Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen}>
          ...
          <SheetContent
            side={"bottom"}
            className="w-full border px-4 h-[100%] sm:h-auto"
          >
            <SheetHeader>
            ...
            <div>
              ...
              {userDetails && (
                <>
                  ...
                  <Link
                    href={"/profile"}
                    className="text-zinc-50 flex items-center gap-3"
                  >
                    perfil
                  </Link>
                  <Link
            ...
            {!userDetails && (
              <SheetFooter
                className={` fixed bottom-0 left-0 right-0 bg-primary p-5 `}
              >
                <Link
                  href={"/login"}
                  className="text-zinc-50 flex items-center gap-3"
      ...
      <AlertDialog open={isOpenDialog} onOpenChange={setIsOpenDialog}>
        <AlertDialogContent className="w-[95%]">
          <AlertDialogHeader>
            <AlertDialogTitle>Crie um anúncio gratuito</AlertDialogTitle>
            <AlertDialogDescription>
              Para criar um anúncio, faça login ou cadastre-se. Clique em
              'Prosseguir' para continuar.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="flex flex-row-reverse w-full items-center justify-center gap-5">
            ...
            <Link
              href={"/login"}
              className="flex-1 mt-2 p-2 rounded-md text-center sm:mt-0 bg-primary text-zinc-50"
            >
              Prosseguir
            </Link>
          </AlertDialogFooter>
